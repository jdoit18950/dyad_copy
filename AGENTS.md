...
- High-level architecture: see [docs/architecture.md](docs/architecture.md). The local agent (Agent v2) is described in [docs/agent_architecture.md](docs/agent_architecture.md).
- Dual licensing: code outside `src/pro/` is Apache 2.0; code inside `src/pro/` is Fair Source License (FSL). Contributions to `src/pro` are governed by FSL — keep the boundary clean and avoid leaking pro-only logic into OSS modules.

## Codebase structure

Top-level layout (only the things you'll touch most often):

| Path                                     | What lives there                                                                                                                                                                                                                                                        |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/main.ts`                            | Electron main-process entry point.                                                                                                                                                                                                                                      |
| `src/main/`                              | Main-process modules (deep link queue, settings, pro entitlement).                                                                                                                                                                                                      |
| `src/preload.ts`                         | Preload script that exposes the IPC bridge to the renderer.                                                                                                                                                                                                             |
| `src/renderer.tsx`                       | Renderer-process entry point. Routes via TanStack Router (`src/router.ts`, `src/routes/`).                                                                                                                                                                              |
| `src/ipc/`                               | IPC layer. `handlers/` are main-process handlers, `contracts/` define typed channels, `processors/` apply LLM-generated changes (e.g. `response_processor.ts`), `preload/` mirrors handler types into the renderer. See [rules/electron-ipc.md](rules/electron-ipc.md). |
| `src/pages/`                             | Top-level page components mounted by the router (chat, apps, app-details, hub, library, media, settings, themes).                                                                                                                                                       |
| `src/routes/`                            | TanStack Router route definitions. Match by filename to entries in `src/pages/`.                                                                                                                                                                                        |
| `src/components/`                        | Renderer UI components. Use Base UI (`@base-ui/react`), never Radix.                                                                                                                                                                                                    |
| `src/hooks/`                             | React hooks, including TanStack Query hooks that wrap IPC calls.                                                                                                                                                                                                        |
| `src/atoms/`                             | Jotai atoms for client-side state. Test patterns in [rules/jotai-testing.md](rules/jotai-testing.md).                                                                                                                                                                   |
| `src/db/`                                | Drizzle ORM `schema.ts` and SQLite client. Migrations live in `drizzle/`. See [rules/database-drizzle.md](rules/database-drizzle.md).                                                                                                                                   |
| `src/prompts/`                           | LLM system prompts (`system_prompt.ts`) and `<dyad-*>` tag specifications consumed by `ipc/processors`.                                                                                                                                                                 |
| `src/pro/`                               | Fair-source (FSL) pro features. Has its own `LICENSE`, `CONTRIBUTING.md`, and `main/`, `shared/`, `ui/` subtrees.                                                                                                                                                       |
| `src/shared/`                            | Pure shared utilities/types used across main and renderer (no Electron-specific imports).                                                                                                                                                                               |
| `src/store/`                             | Cross-cutting client store.                                                                                                                                                                                                                                             |
| `src/utils/`, `src/lib/`                 | Renderer-side helpers; keep main-process logic out.                                                                                                                                                                                                                     |
| `src/neon_admin/`, `src/supabase_admin/` | Integrations for Neon (Postgres) and Supabase (Edge Functions, auth). See [rules/supabase-functions.md](rules/supabase-functions.md).                                                                                                                                   |
| `src/__tests__/`                         | Unit tests for renderer/main code (colocated tests also exist alongside source).                                                                                                                                                                                        |
| `drizzle/`                               | Generated SQL migrations. Do not hand-edit — run `npm run db:generate`.                                                                                                                                                                                                 |
| `e2e-tests/`                             | Playwright E2E specs. Tests run against the built app (`npm run build` first). See [rules/e2e-testing.md](rules/e2e-testing.md).                                                                                                                                        |
| `workers/tsc/`                           | TypeScript worker (separate `tsc` build, see `npm run ts:workers`).                                                                                                                                                                                                     |
| `worker/`                                | Renderer-side worker/client scripts (visual editor client, screenshot client, proxy server, etc.).                                                                                                                                                                      |
| `scaffold/`                              | Starter template scaffolded into new Dyad apps (its own `package.json`, Vite, Tailwind, ESLint config).                                                                                                                                                                 |
| `packages/@dyad-sh/`                     | First-party npm packages published under the `@dyad-sh` scope.                                                                                                                                                                                                          |
| `testing/`                               | Local fake servers (LLM, MCP) used by E2E and manual testing.                                                                                                                                                                                                           |
| `scripts/`                               | Build/release and CI helper scripts (version bump, PR review, codex, etc.).                                                                                                                                                                                             |
| `docs/`                                  | Human-facing docs: `architecture.md`, `agent_architecture.md`, `i18n.md`, and `adrs/`.                                                                                                                                                                                  |
| `rules/`                                 | AI-assistant rule files. Read the relevant one _before_ editing in that area.                                                                                                                                                                                           |
| `plans/`                                 | Long-form design plans (often produced by `dyad:swarm-to-plan`).                                                                                                                                                                                                        |
| `.github/workflows/`                     | CI. Several workflows invoke `anthropics/claude-code-action`; if you edit those, see [rules/claude-github-workflows.md](rules/claude-github-workflows.md).                                                                                                              |

Top-level config worth knowing about: `forge.config.ts` (Electron Forge packaging), `vite.main.config.mts` / `vite.preload.config.mts` / `vite.renderer.config.mts` / `vite.worker.config.mts` (per-process Vite configs), `drizzle.config.ts`, `playwright.config.ts`, `vitest.config.ts`, `tsconfig.app.json` (main app — used by `npm run ts:main` via `tsgo`).
